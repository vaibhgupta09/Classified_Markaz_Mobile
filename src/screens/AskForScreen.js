...
export default class AskForScreen extends PureComponent {
    ...
    getRandomPost=()=>{
        fetch('http://www.mocky.io/v2/5d9fe7853000001269524a28')
    .then((response) => response.json())
    .then((responseJson) => {
      console.log(responseJson)
      if(responseJson.status=true)
      {
          this.setState({askforList:responseJson.AskForListView})
      }
        ...
        >
            ...
            <View style={styles.container}>
                    ...
                    <FlatList
                        ...
                        renderItem={({item}) => 
                         <TouchableOpacity style={{width:100,height:100,marginLeft:10}}>
                        ...
                        <Text style={{fontSize:23,color:'grey',fontWeight:'bold',textAlign:'center'}}>MOST RECENT ASK FOR QUOTATIONS</Text>
                        ...
                        contentContainerStyle={{marginBottom:120}}
                                data={this.state.askforList}
                                renderItem={({item}) => 
                                <TouchableOpacity onPress={()=>Actions.push('AskForDetail')} style={{width:width,height:height/4,justifyContent:'center',alignItems:'center',marginTop:15}}>
                                <View style={{width:'85%',backgroundColor:'#f5f5f5',borderRadius:8,height:'90%',flexDirection:'column',justifyContent:'center',alignItems:'center'}}>
                                <View style={{width:'100%',height:'15%'}}>
                                <Text style={{fontSize:16,color:'grey',fontWeight:'bold',marginTop:5,marginLeft:10}}>{item.productName}</Text>
                                </View>
                                <View style={{width:'100%',height:'85%',flexDirection:"row"}}>
                               
                                  <View style={{width:'85%',height:"100%"}}>
                                                   <Text style={{fontSize:14,color:'grey',marginTop:10,marginLeft:10}}>{item.description}</Text>
                                                
      
                                                   </View>
      
                                         <View style={{width:'15%',height:"100%",justifyContent:"flex-end",alignItems:'flex-end'}}>
                                                   <Image source={Capture} style={{width:'50%',height:'30%',resizeMode:'contain',marginBottom:10,marginRight:10}}/>     
                                                       </View>
